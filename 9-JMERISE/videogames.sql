...
--Instructions from Anousone - Live terminal on the terminal--

--
-- Afficher toutes les données du tableau `games`.
--
SELECT * FROM `games`;
--
-- Afficher les noms des jeux sans les doublons.
--
SELECT DISTINCT `g_name`
FROM `games`;
--
-- Afficher les colonnes [nom], [mode de jeu], [date de sortie], [pegi] sans les doublons.
--
SELECT DISTINCT `g_name`, `g_mode`, `g_published_at`, `g_pegi`
FROM `games`;
--
-- Afficher les colonnes [nom], [mode de jeu], [date de sortie], [pegi] sans les doublons. Classer les jeux par ordre alphabétique.
--
SELECT DISTINCT `g_name`, `g_mode`, `g_published_at`, `g_pegi`
FROM `games`
ORDER BY `g_name`;
--
-- Afficher les colonnes [nom], [mode de jeu], [date de sortie], [pegi] sans les doublons. Classer les jeux par date de sortie du plus récent au plus ancien.
--
SELECT DISTINCT `g_name`, `g_mode`, `g_published_at`, `g_pegi`
FROM `games`
ORDER BY `g_published_at` DESC;
--
-- Afficher les 10 derniers jeux sortis.
--
SELECT DISTINCT `g_name`, `g_mode`, `g_published_at`, `g_pegi`
FROM `games`
ORDER BY `g_published_at` DESC
LIMIT 10;
--
-- Afficher les jeux ne disposant que d'un seul mode.
--
SELECT DISTINCT `g_name`, `g_mode`
FROM `games`
WHERE `g_mode` = 'Solo'
OR `g_mode` = 'Multijoueur';
--
-- Afficher les jeux qui sortis entre 2015 et 2020.
--
SELECT DISTINCT `g_name`, `g_published_at`
FROM `games`
WHERE `g_published_at`
BETWEEN '2015-01-01' AND '2020-12-31';
--
--  Afficher toutes les colonnes des jeux sortis entre 2015 et 2020.
--
SELECT * FROM `games`
WHERE `g_published_at`
BETWEEN '2015-01-01' AND '2020-12-31';
